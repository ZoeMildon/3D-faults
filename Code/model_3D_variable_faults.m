...
% check for missig data and data types to avoid errors caused by input
if isnumeric(faults.dip) == false
    faults.dip = str2double(faults.dip);
end
if isnumeric(faults.rake) == false
    faults.rake = str2double(faults.rake);
end
if isnumeric(faults.dip_dir) == false
    faults.dip_dir = str2double(faults.dip_dir);
end
if isnumeric(faults.len) == false
    faults.len = str2double(faults.len);
end

for j = 1:length(faults.dip)
    if ismissing(faults.dip(j)) == true
