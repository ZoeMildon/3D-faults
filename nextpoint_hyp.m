function [x_next,y_next,last_point,a] = nextpoint_hyp2_ZKM(x_start,y_start,grid_size,utm_lon,utm_lat,b)
...
if a~=b && isempty(a)==1
    a=find(utm_lon>x_start,1,'first'); % fixes for when grid_size is smaller than the first kml segment
end
...
if isempty(a)==1  % setting the end of the grid to be the end of the kml fault trace
    ...
    while found<1
        ...
        if hyp_dist>grid_size
            found=1;
            if counter==0
%               x_next=x_start+(grid_size/hyp_dist)*(utm_lon(a)-x_start);
%               y_next=y_start+(grid_size/hyp_dist)*(utm_lat(a)-y_start); 
                if x_start<=utm_lon(a)
                    x_next=x_start+(grid_size/hyp_dist)*(utm_lon(a)-x_start);
                else
                    x_next=x_start-(grid_size/hyp_dist)*(x_start-utm_lon(a));
                end
                if y_start>=utm_lat(a)
                    y_next=y_start-(grid_size/hyp_dist)*(y_start-utm_lat(a));
                else
                    y_next=y_start+(grid_size/hyp_dist)*(utm_lat(a)-y_start);
                end
            else
                ...
                % horizontal lines.
                l1=[utm_lon(a-1),utm_lat(a-1),0]-[x_start,y_start,0];
                l2=[utm_lon(a-1),utm_lat(a-1),0]-[utm_lon(a),utm_lat(a),0];
                e=acos(dot(l1,l2)/(norm(l1)*norm(l2)))*180/pi;
                f=rad2deg(asin(norm(l1)*sind(e)/grid_size));
                g=180-e-f;
                t=(grid_size*sind(g)/sind(e))/norm(l2);
%               x_next=utm_lon(a-1)+t*(utm_lon(a)-utm_lon(a-1));
%               y_next=utm_lat(a-1)+t*(utm_lat(a)-utm_lat(a-1));
            
                if utm_lon(a)>=utm_lon(a-1)
                    x_next=utm_lon(a-1)+t*(utm_lon(a)-utm_lon(a-1));
                else
                    x_next=utm_lon(a-1)-t*(utm_lon(a-1)-utm_lon(a));
                end
                if utm_lat(a)<=utm_lat(a-1)
                    y_next=utm_lat(a-1)-t*(utm_lat(a-1)-utm_lat(a));
                else
                    y_next=utm_lat(a-1)+t*(utm_lat(a)-utm_lat(a-1));
                end
